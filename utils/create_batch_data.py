...
def get_data_batch(
    file_list, labels_nums, batch_size=None, shuffle=False, one_hot=False
):
    """
    ...
    while True:
        ...
        batch_indexMat_labels = indexMat_labels[
            batch_indices
        ]  # 使用下标查找，必须是ndarray类型类型
        indexMat_labels = np.delete(indexMat_labels, batch_indices, axis=0)
        height = indexMat_labels.shape[0]

        # 将数据分割成indexMat和labels
        batch_labels, batch_indexMat = create_word2vec.split_labels_indexMat(
            batch_indexMat_labels
        )
        # 是否进行独热编码
        if one_hot:
            batch_labels = batch_labels.reshape(len(batch_labels), 1)
            onehot_encoder = preprocessing.OneHotEncoder(
                sparse=False, categories=[range(labels_nums)]
            )
...
if __name__ == "__main__":
    ...
    batch = get_data_batch(
        file_list, labels_nums=14, batch_size=6, shuffle=False, one_hot=False
    )
    for i in range(iter):
        print("**************************")
