...
def save_multi_file(
    ...
    for i in range(batchNum):
        ...
        # 读取文件内容，字词分割
        batch_content = files_processing.read_files_list(
        ...
        print(
            "step:{}/{}, save:{}, data.shape{}".format(
                i, batchNum, filename, labels_indexMat.shape
            )
        )
...
def pos_neg_test():
    ...
    sentences, labels = files_processing.load_pos_neg_files(
        positive_data_file, negative_data_file
    )
    # embedding_test(positive_data_file,negative_data_file)
    sentences, max_document_length = segment.padding_sentences(
        sentences, "<PADDING>", padding_sentence_length=190
    )
...
if __name__ == "__main__":
    ...
    # 将数据划分为train val数据集
    train_files, train_label, val_files, val_label = (
        files_processing.split_train_val_list(
            files_list, label_list, facror=0.9, shuffle=True
        )
    )
