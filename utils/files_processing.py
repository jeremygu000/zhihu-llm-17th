...
def read_files_list(files_list, max_sentence_length, padding_token="<PAN>"):
    content_list = segment.segment_files_list(
        files_list, stopwords=[], segment_type="word"
    )
    content_list = segment.padding_sentences(
        content_list,
...
def read_and_clean_zh_file(input_file, output_cleaned_file=None):
    ...
    lines = [
        segment.clean_str(segment.seperate_line(line.decode("utf-8"))) for line in lines
    ]
...
if __name__ == "__main__":
    ...
    contents_list = read_files_list(
        files_list, max_sentence_length=190, padding_token="<PAD>"
    )
