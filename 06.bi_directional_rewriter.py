...
if __name__ == "__main__":
    br = BidirectionalRewriter()

    # 示例：Query2Doc
    q = "如何提高深度学习模型的训练效率？"
    pseudo_doc = br.query2doc(q)
    print("=== Query2Doc（伪文档）===\n", pseudo_doc, "\n")

    # 示例：Doc2Query
    doc = (
        "本文介绍了深度学习模型训练的优化策略，包括："
        "1) 使用 AdamW 或 LAMB 等优化器；"
        "2) 采用混合精度训练减少显存占用；"
        "3) 使用分布式训练技术加速大规模训练；"
        "4) 优化数据管线与预处理；"
        "5) 使用合适的学习率与正则化方案。"
    )
    queries = br.doc2queries(doc, n=5)
    print("=== Doc2Query（生成问题）===\n", queries, "\n")

    # 示例：组合
    combo = br.rewrite_both(q, [doc], per_doc_n=4)
