...
def get_top_n_words(corpus, n=1, k=None):
    ...
    vec = CountVectorizer(ngram_range=(n, n), stop_words=list(ENGLISH_STOP_WORDS)).fit(
        corpus
    )
...
df1.groupby("desc").sum()["count"].sort_values().plot(
    kind="barh", title="Top 20 words in hotel descriptions after removing stop words"
)
plt.show()
